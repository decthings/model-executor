...
impl ModelExecutor {
    ...
    /// Get a tokio::process::Command which has been setup to spawn the model. Also sets up a Unix
    /// socket listener, and returns a Future which resolves once the child has connected to us
    /// using the Unix socket. Using this connection you can call RPC methods and listen for events
    /// on the child.
    ///
    ...
    pub fn get_model_command<S: AsRef<str>>(
        &self,
...
exports.default = {}",
        ...
        let (mut command, rpc_fut) =
            model_executor.get_model_command::<&str>(ModelToExecute::NodeJs { flags: &[] });
                    ...
                    async {
                        ...
                        impl rpc::ChildEventCallbacks for Callback {
                            fn on_event<'a>(
                                ...
                                additional_segments: Option<rpc::AdditionalSegments<'a>>,
                            ) -> Pin<Box<dyn Future<Output = bool> + Send + 'a>>
                            {
                                Box::pin(async move {
                                    ...
                                    assert!(additional_segments.is_none());
